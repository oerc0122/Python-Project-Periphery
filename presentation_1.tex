...
\newcommand{\foldt}[1]{\texttt{\detokenize{#1}}}
...
\begin{frame}{Making a package}
    \begin{itemize}[<+->]
        ...
        \item{}In Python it makes a folder accessible for \lstinline{import}.
    \end{itemize}
...
\begin{frame}{Making a package}
    ...
    \begin{alertblock}<+>{Add some code}
...
\begin{frame}{Making a project}
    ...
    \begin{block}<2->{TOML History}
        TOML (Tom's Own Markup Language) is a standardised format designed to replace the non-standardised \cmd{ini} format configurations.
    \end{block}
...
\begin{frame}{The pyproject.toml}
    \lstinputlisting[basicstyle=\tiny]{example.toml}
...
\begin{frame}{build-system}
    \lstinputlisting[linerange={1-3}]{example.toml}
    \begin{itemize}[<+->]
...
\begin{frame}{project}
    \lstinputlisting[linerange={5-15}]{example.toml}
    \begin{itemize}
...
\begin{frame}{Webpages}
    \lstinputlisting[linerange={18-20}]{example.toml}
    \begin{itemize}[<+->]
...
\begin{frame}{Dynamic}
    \lstinputlisting[linerange={5,16,21-23}]{example.toml}
    \begin{itemize}[<+->]
...
\begin{frame}{IDE Ahoy}
    \begin{itemize}[<+->]
        ...
        \item{}It also tells you the \cmd{type} it should be (\cmd{int}, \cmd{float}, etc.).
    \end{itemize}
\end{frame}
...
\begin{frame}{conf.py}
    \begin{itemize}[<+->]
        \item{}This is an auto-generated Python file with instructions for building the docs.
        ...
        \item{}Sphinx is a fully extensible package. We'll be using some of these later.

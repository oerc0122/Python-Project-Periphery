...
\begin{frame}{Before we start}
    \begin{itemize}
        \item{}You may need to create a virtual environment to be able to install things.
    \end{itemize}  

    \begin{alertblock}{Set up Virtual environment}
        \begin{itemize}
            \item{}Run \cmd{python -m venv <project>} (where project is the name you want to use).
            \item{}From \filet{Scripts} in \filet{<project>}, run \cmd{activate.bat}.
        \end{itemize}
    \end{alertblock}
...
\begin{frame}[fragile]{Docstring magic}
    \begin{itemize}
        \item{}But what if we add files? 
        \item{}Wouldn't it be tedious to run this every time?
        \item{}Time to use another extension!
    \end{itemize}

     \begin{alertblock}{Try it now}
        \begin{itemize}
            \item{}Add a new extensions \kw{sphinx.ext.apidoc}.
            \item{}Add a new configuration variable which looks like the following.
            \item{}\textbf{Note:} Directories are relative to the \filet{conf.py}
        \end{itemize}
        \begin{lstlisting}
apidoc_modules = [
    {"path": "../../polysolve", 
